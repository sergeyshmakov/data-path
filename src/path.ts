import type { Path, BasePath, Segment, PathExpression } from "./types.js";
...
export function path<T, V = unknown>(expr: PathExpression<T, V>): Path<T, V, string>;
...
export function path<T, V = unknown>(
    ...
    expr?: PathExpression<unknown, V> | { segments: readonly Segment[] } | unknown,
): Path<T, V, string> {
    if (!baseOrExpr) {
    ...
    if (typeof baseOrExpr === "function") {
        ...
        const segments: Segment[] = (result as Record<symbol, unknown>)?.[PATH_SEGMENTS] as Segment[] ?? [];
...
export function unsafePath<T>(raw: string): Path<T, unknown, string> {
    const segments: Segment[] = raw
        ? raw
              .split(".")
              .map((s) =>
                  s === "" ? s : Number.isNaN(Number(s)) ? s : Number(s),
              )
        : [];
