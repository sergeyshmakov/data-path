...
export function BlogList({ posts }: { posts: BlogPost[] }) {
	// The blog ships before its first post, so the empty case is a real state
	// here rather than a defensive branch — it renders on /blog/ today.
	if (posts.length === 0) {
		return (
			<p className="rounded-xl border border-dashed bg-fd-card p-6 text-fd-muted-foreground">
				No posts yet.
			</p>
		);
	}
