...
Accept a typed `Path` as a prop so the component is self-documenting and type-checked:
...
function TextField<T>({ label, fieldPath, register }: FieldProps<T>) {
  return (
    <label>
      {label}
...
- [Runtime Variables](/data-path/guides/runtime-variables/) — how lambdas capture loop indices
- [Data access](/data-path/guides/data-access/) — `get`, `set`, `update` for non-form use cases
