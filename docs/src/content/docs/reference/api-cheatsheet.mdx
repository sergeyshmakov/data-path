...
| Method | Signature | Description |
|--------|-----------|-------------|
| `.to(lambda \| path)` | `Path<T, U> \| TemplatePath<T, U>` | Extend the path. Result is a `TemplatePath` when the `relative` argument carries wildcards; otherwise `Path` |
| `.parent()` | `Path<T, unknown> \| null` | Remove the last segment; `null` at root |
| `.merge(other)` | `Path<T, U> \| TemplatePath<T, U>` | Overlap-aware concatenation. Result is a `TemplatePath` when `other` carries wildcards |
