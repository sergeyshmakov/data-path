...
| Method | Signature | Description |
|--------|-----------|-------------|
| `.to(lambda \| path)` | `Path<T, U>` | Extend the path to a deeper property |
| `.parent()` | `Path<T, unknown> \| null` | Remove the last segment; `null` at root |
| `.merge(other)` | `Path<T, U>` | Overlap-aware concatenation |
...
Template paths also support `.to()` and `.merge()` — both return `TemplatePath<T, U>` to preserve expansion.
...
| `.includes(other)` | `boolean` | `true` if `other` is a prefix of this path |
